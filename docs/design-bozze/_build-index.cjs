// Costruisce un unico index.html che incapsula le 5 bozze (iframe srcdoc) + barra di navigazione.
const fs = require('fs');
const path = require('path');
const dir = __dirname;

const files = [
  ['b1', '1-editoriale.html',     '01 · Editoriale'],
  ['b2', '2-brutalista.html',     '02 · Brutalista'],
  ['b3', '3-tecnico-sepia.html',  '03 · Tecnico seppia'],
  ['b4', '4-crt-ambra.html',      '04 · CRT ambra'],
  ['b5', '5-organico-caldo.html', '05 · Organico'],
];
...
<title>Geometria del Taxi — 5 bozze di design</title>
<style>
  :root { --bar: 56px; }
...
<body>
  <iframe id="stage" title="Anteprima bozza"></iframe>
  <nav id="bar" aria-label="Navigazione fra le bozze">
  ...
  <script>
    ...
    function load(i) {
      ...
      label.innerHTML = 'Bozza <b>' + PAGES[cur][1] + '</b>';
...
console.log('Scritto index.html (' + (out.length / 1024).toFixed(0) + ' KB)');
